...
class PaymentsRelationManager extends RelationManager
{
    ...
    protected function validatePaymentAmount(Action $action): void
    {
        ...
        $purchase = $this->getOwnerRecord()->refresh();
        $balanceDue = max(0, (float) ($purchase->balance_due ?? 0));

        $editingRecord = $action->getRecord();
        $editingAmount = $editingRecord instanceof PurchasePayment
        ...
        if ($maxAllowed <= 0) {
            Notification::make()
                ->title('Purchase ini sudah lunas')
                ->warning()
                ->send();
    ...
    {
        app(RecalculatePurchasePaymentSummary::class)->handle((int) $this->getOwnerRecord()->getKey());
