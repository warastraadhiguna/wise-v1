...
class PurchasePaymentValidator
{
    ...
    public static function assertCashPaymentMatchesGrandTotal(
        ...
        if (abs($paymentAmount - $grandTotal) < 0.0001) {
            return;
        }

        throw ValidationException::withMessages([
            'payment_amount' => 'Untuk pembayaran tunai, jumlah bayar harus sama dengan grand total ('
    ...
    public static function calculateGrandTotalFromFormData(array $data): float
    {
        ...
        foreach (($data['details'] ?? []) as $detail) {
            $detailsSubtotal += PurchaseTotalsCalculator::lineTotal(
                $detail['qty'] ?? 0,
                $detail['price'] ?? 0,
        ...
        return PurchaseTotalsCalculator::grandTotal(
            $detailsSubtotal,
